...
export default function productsAllPage() {
    ...
    // const [tab, setTab] = useState({typeId})
    // eslint-disable-next-line react-hooks/rules-of-hooks
                    ...
                    <div className={"container grid grid-cols-12 gap-4 mx-auto border-b  border-zinc-400 "}>
                        <div className={"col-span-3"}>
                            <div
                                ...
                                <a href={"/all-products"}
                                   className={`${true ? "bg-black text-white" : ""} mb-2 ml-2 items-center inline-flex border border-black rounded-lg px-4 py-1`}>
                                    {true ? <CheckIcon className={"w-4 h-4 text-white ml-2"}/> :
                                        <div className={"h-4"}/>}
                                    نمایش همه
                                </a>
                                {types.map((item, i) =>
                                    <a href={`/all-products/${item.id}`}
                                       className={`${false ? "bg-black text-white" : ""} mb-2 ml-2 inline-flex items-center border border-black rounded-lg px-4 py-1`}
                                       key={i}>
                        ...
                        <div className={"col-span-9"}>
                            ...
                            <div className={"grid grid-cols-3  gap-2"}>
                                {products.map((item, i) => <ProductCard withoutItems item={item} key={i}/>)}
                            </div>
                        </div>
                    </div>
